...
DFRobot_IntelligentGasSensor::DFRobot_IntelligentGasSensor(Stream *s, uint8_t slaveAddr, int dePin) : DFRobot_RTU(s, dePin), _slave(slaveAddr), _readRetries(DFROBOT_IGS_DEFAULT_READ_RETRIES)
{
...
void DFRobot_IntelligentGasSensor::applyTimestampToMeasure(DFRobot_IntelligentGasSensorMeasure_t *m, const uint16_t *t)
{
  ...
  if (m->tsYear < 2000 || m->tsYear > 2099 || m->tsMonth < 1 || m->tsMonth > 12 || m->tsDay < 1 || m->tsDay > 31 || m->tsHour > 23 || m->tsMinute > 59 || m->tsSecond > 59) {
    m->timestamp[0] = '\0';
    return;
  }
  (void)snprintf(m->timestamp, sizeof(m->timestamp), "%04u-%02u-%02u %02u:%02u:%02u", (unsigned)m->tsYear, (unsigned)m->tsMonth, (unsigned)m->tsDay, (unsigned)m->tsHour, (unsigned)m->tsMinute, (unsigned)m->tsSecond);
}
...
void DFRobot_IntelligentGasSensor::fillLastMeasureFromInputRegs(DFRobot_IntelligentGasSensorMeasure_t *out, const uint16_t *t, uint16_t regCount)
{
  if (out == nullptr)
    return;
...
float DFRobot_IntelligentGasSensor::getConcentrationFloat(void) const
{
  ...
  for (uint8_t i = 0; i < lastMeasure.decimalPoint; i++) v *= 0.1;
  return (float)v;
}
