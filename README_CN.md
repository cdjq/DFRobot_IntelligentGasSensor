...
硬件接线图（以 ESP32 为例；传感器对外可使用 **RS-485 A/B**，TX/RX/DE 接 **UART 转 RS485 模块**，也可以直连传感器）<br>
...
| `setinterval <1-60>` | 设置 CSV 记录间隔（分钟），**写入 EEPROM** |
| `logger on/off`  | 开启/关闭 CSV 记录（**写入 EEPROM**） |
| `repairfs` | 读-写 USB 存储设备，用于修复文件系统 |
